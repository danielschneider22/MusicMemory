...
export default function ArtistCard() {
  const [artist, setArtist] = useState<{ label: string } | null>(null);
  ...
  const setArtistsAndSongs = (
    ...
    setData!((prevData) => {
      const newSongList = filterUniqueSongs([
        ...
        return { ...track, type: "Artist Picker" };
      });
      return { ...prevData, songList: newSongList };
    });
    setGeneralInfoData!({
      ...generalInfoData,
      artists: [...generalInfoData.artists, artist!.label],
    });
    setArtist(null);
  };
  ...
  const addArtistSongs = () => {
    const artistSongs = itunesSongs.filter((iSong) =>
      iSong.artist.toLowerCase().includes(artist!.label.toLowerCase())
    );
    if (showAllOfArtist || artistSongs.length <= numToGenerate) {
      ...
      spotifySearchTrack(`artist:${artist!.label}`, 50).then((tracks) => {
        if (tracks) {
          let spotifyTracks = tracks.filter(
            (track) =>
              track.artist.toLowerCase() === artist!.label.toLowerCase()
          );
          const artistSongs = itunesSongs.filter((iSong) =>
            iSong.artist.toLowerCase().includes(artist!.label.toLowerCase())
          );
          ...
          let filteredSongs = artistSongs.filter((song) =>
            spotifyTracks.some((t) => t.name.includes(song.title))
          );

          // Sort the filtered songs by popularity
          filteredSongs.sort((a, b) => {
            return (
              spotifyTracks.find((t) =>
                t.name.toLowerCase().includes(b.title.toLowerCase())
              )!.popularity -
              spotifyTracks.find((t) =>
                t.name.toLowerCase().includes(a.title.toLowerCase())
              )!.popularity
            );
          });
          ...
          const remainingSongs = artistSongs.filter(
            (song) =>
              !spotifyTracks.some(
                (t) => song.title.toLowerCase() === t.name.toLowerCase()
              )
          );
  ...
  function setArtists(artists: string[], removedArtist: string) {
    const songList = data.songList.filter(
      (song) => !song.artist.toLowerCase().includes(removedArtist.toLowerCase())
    );
  ...
  let css =
    "peer-focus:font-medium absolute text-sm  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 disabled:text-gray-200 text-gray-400";
  if (showAllOfArtist) css += " text-gray-600";
  return (
      ...
      <div className="grid md:grid-cols-1 md:gap-6">
        <Autocomplete
          ...
          onInputChange={(event, newInputValue) => {
            setInputValue(newInputValue); // Update the input text as the user types
          }}
          blurOnSelect={true}
          options={artistList}
          value={artist}
        ...
        <div className="relative z-0 w-full group text-left pl-8 pr-8">
          <input
            disabled={showAllOfArtist}
            type="number"
            max={50}
            value={numToGenerate}
            onChange={(ev) =>
              setNumToGenerate(Math.min(Number(ev.currentTarget.value), 50))
            }
            name="floating_num_to_generate"
            id="floating_num_to_generate"
            className="block py-2.5 px-0 w-full text-md bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer  text-white disabled:text-gray-600"
            placeholder=" "
            required
          />
          <label htmlFor="floating_num_to_generate" className={css}>
            # Songs to Generate
          </label>
        </div>
        <div className="relative z-0 w-full group text-left pl-8 pr-8">
          <input
            ...
            type="checkbox"
            checked={showAllOfArtist}
            onChange={(ev) => toggleAllOfArtist(ev.target.checked)}
            name="floating_all_of_artist"
            id="floating_all_of_artist"
            placeholder=" "
            required
          />
          <label
            htmlFor="floating_all_of_artist"
            className="cursor-pointer peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 ml-3"
          >
            Add all songs by artist
          </label>
        </div>
        <TypeList
          setList={setArtists}
          list={generalInfoData.artists}
          className={"pl-4 pr-4"}
        />
        <button
          disabled={!artist || !artist.label}
          onClick={() => addArtistSongs()}
          id="button"
          type="button"
          className="disabled:bg-indigo-800 bg-indigo-600 shadow-xl hover:bg-indigo-500 text-white font-bold rounded-full p-4 w-48 mx-auto"
        >
          Add Songs
        </button>
      </div>
