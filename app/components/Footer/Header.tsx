...
const Footer = ({ gridAPI }: { gridAPI: any }) => {
  ...
  const exportItunesPlaylist = () => {
    const quotedElements: string[] = data.songList.map(
      (element) => `"${element.title.replace(/"/g, '\\"')}"`
    );
  ...
  const bulkUploadCSVSongs = (vals: { [key: string]: string }[]) => {
    ...
    vals.forEach((song) => {
      let foundSongs: Song[] = itunesSongs.filter((iSong) => {
        return Object.keys(song).every(
          (key) =>
            key === "OTHER" ||
            removePunctuationAndNormalize((iSong as any)[key]).includes(
              removePunctuationAndNormalize(song[key])
            )
        );
      });
      ...
      if (
        foundSongs.length === 0 &&
        !["", "song", "title", "name"].some(
          (badName) => song.title.toLowerCase() === badName
        )
      ) {
        foundSongs = itunesSongs.filter((iSong) =>
          removePunctuationAndNormalize(iSong.title).includes(
            removePunctuationAndNormalize(song.title)
          )
        );
      }
    ...
    if (notFoundSongs.length) {
      window.alert(
        "Could not find the following songs: " + notFoundSongs.join(", ")
      );
    }
  };
  ...
  const bulkUploadSongs = (text: string) => {
    ...
    text.split("\n").forEach((song) => {
      const foundSongs = itunesSongs.filter((iSong) =>
        removePunctuationAndNormalize(iSong.title).includes(
          removePunctuationAndNormalize(song)
        )
      );
      if (foundSongs.length === 1) {
        songList.push(foundSongs[0]);
      } else {
        issueSongs.push(foundSongs);
      }
    });

    const newSongList = filterUniqueSongs([...data.songList, ...songList]);
    ...
    <>
      <div className="w-screen flex justify-center space-x-2 pt-4 border-b border-b-gray-800">
        <button
          ...
          type="button"
          className="text-white p-3 w-48 bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-m px-5 py-2.5  mb-2 dark:bg-teal-800 dark:hover:bg-teal-700 dark:focus:ring-gray-700 dark:border-gray-700"
        >
          Save User
        </button>
        <input
        ...
        <button
          ...
          type="button"
          className="text-white p-3 w-48 bg-teal-800 hover:bg-teal-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-m px-5 py-2.5  mb-2 dark:bg-sky-800 dark:hover:bg-sky-700 dark:focus:ring-gray-700 dark:border-gray-700"
        >
          Load User
        </button>
        <button
          ...
          id="button"
          type="submit"
          className="bg-purple-600 shadow-xl hover:bg-purple-500 p-3 w-48 font-medium rounded-lg text-m py-2.5  mb-2"
        >
          Bulk Add Songs
        </button>
        <button
          ...
          id="button"
          type="submit"
          className="bg-orange-600 shadow-xl hover:bg-orange-500 p-3 w-48 font-medium rounded-lg text-m py-2.5  mb-2"
        >
          Bulk Add Songs (CSV)
        </button>
        <button
          ...
          id="button"
          type="submit"
          className="bg-green-600 shadow-xl hover:bg-green-500 p-3 w-48 font-medium rounded-lg text-m py-2.5  mb-2"
        >
          Export Songs to CSV
        </button>
        <button
          onClick={() => exportItunesPlaylist()}
          id="button"
          type="submit"
          className="bg-slate-600 shadow-xl hover:bg-slate-500 p-3 w-48 font-medium rounded-lg text-m py-2.5  mb-2"
        >
      ...
      {modalShown && (
        <Modal
          ...
          header={"Bulk Add Songs"}
          onSubmit={(text) => {
            bulkUploadSongs(text);
            setShowModal(false);
          }}
        />
      )}
      {modalCSVShown && (
        <CSVModal
          ...
          placeholder={placeholderText}
          header={"Bulk Add Songs (CSV)"}
          onSubmit={(vals) => {
            bulkUploadCSVSongs(vals);
      ...
      {songChoices.length > 0 && (
        <SongChoicesModal
          songChoice={songChoices[0]}
          selectSong={(song) => {
            selectSong(song);
            setSongChoices(songChoices.slice(1));
          }}
          ignoreChoice={() => setSongChoices(songChoices.slice(1))}
          header={"Bulk Add Songs"}
        />
      )}
    </>
